    ...
    $(document).ready(function(){
        ...
        var table = $('#'+tbl_id).DataTable({
            ...
            columns: [
                ...
                { data: 'image_color_code', 
                    name: 'image_color_code',
                        class: 'text-center', searchable: true, orderable: false, width: '100px'  
                },
                { data: 'image_color_name', 
                    name: 'image_color_name',
                        class: 'text-center', searchable: true, orderable: false, width: '100px'  
                },
                { data: 'image_img_name', 
                    name: 'image_img_name',
        ...
        var hide = [0,1,2,3]; //นับจาก Column
        ...
        var button_add = '<button href="{!!url("backend/$url/$ref_id/image/create/")!!}" class="btn btn-sm btn-grd-primary m-l-10 add" data-toggle="tooltip" data-original-title="คลิ๊กเพื่อเพิ่มข้อมูลรายการใหม่"><i class="icofont icofont-plus add"></i> เพิ่มข้อมูล</button>';
        var button_update = '<button url="{!!url("backend/$url/$ref_id/image")!!}" class="btn btn-sm btn-grd-primary m-l-10 edit" data-toggle="tooltip" data-original-title="คลิ๊กเพื่อแก้ไขข้อมูลจากรายการที่เลือก"><i class="icofont icofont-edit"></i> แก้ไขข้อมูล</button>';
        var button_delete = '<button url="{!!url("backend/$url/$ref_id/image")!!}" class="btn btn-sm btn-grd-danger m-l-10 delete" data-toggle="tooltip" data-original-title="คลิ๊กเพื่อลบข้อมูลจากรายการที่เลือก"><i class="icofont icofont-eraser"></i> ลบข้อมูล</button>';
        ...
        $(document).on('keyup change', 'input.sort', function(){
            window.update_sort(route,token,'cag_product_image','image_id','image_sort',$(this),'../../..');
        });
        //Update Active
        $(document).on('click', 'button.active', function(){
            window.update_active(route,token,'cag_product_image','image_id','image_active',$(this),'../../..');
        });
    });
</script>
@endpush
