...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        var ajax = {
            "url": '{!! route($url.'.datatables_color') !!}', 
            "type": "POST",
            'headers': {
        ...
        var table = $('#'+tbl_id).DataTable({
            ...
            columns: [
                ...
                { data: 'color_code', 
                    name: 'color_code',
                        class: 'text-center info', searchable: true, orderable: false, width: '100px'  
                },
                { data: 'color_name', 
                    name: 'color_name',
                ...
                { data: 'color_created_at',  
                    name: 'color_created_at', 
                        class: ' text-center', searchable: true, orderable: true, width: '100px'   
                },
                { data: 'color_updated_at',  
                    name: 'color_updated_at', 
                        class: ' text-center', searchable: true, orderable: true, width: '100px' 
                }            
            ],
            aaSorting: sorting,
        ...
        var button_add = '<button href="{!!url("backend/$url/$ref_id/color/create/")!!}" class="btn btn-sm btn-grd-primary m-l-10 add" data-toggle="tooltip" data-original-title="คลิ๊กเพื่อเพิ่มข้อมูลรายการใหม่"><i class="icofont icofont-plus add"></i> เพิ่มข้อมูล</button>';
        var button_update = '<button url="{!!url("backend/$url/$ref_id/color")!!}" class="btn btn-sm btn-grd-primary m-l-10 edit" data-toggle="tooltip" data-original-title="คลิ๊กเพื่อแก้ไขข้อมูลจากรายการที่เลือก"><i class="icofont icofont-edit"></i> แก้ไขข้อมูล</button>';
        var button_delete = '<button url="{!!url("backend/$url/$ref_id/color")!!}" class="btn btn-sm btn-grd-danger m-l-10 delete" data-toggle="tooltip" data-original-title="คลิ๊กเพื่อลบข้อมูลจากรายการที่เลือก"><i class="icofont icofont-eraser"></i> ลบข้อมูล</button>';
        ...
        $(document).on('keyup change', 'input.sort', function(){
            window.update_sort(route,token,'cag_product_color','color_id','color_sort',$(this),'../../..');
        });
        //Update Active
        $(document).on('click', 'button.active', function(){
            window.update_active(route,token,'cag_product_color','color_id','color_active',$(this),'../../..');
        });
    });
</script>
@endpush
