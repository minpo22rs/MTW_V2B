...
class General {
	...
	static function hide_name($text = '')
    {
		$string = $text;
		$length = General::mb_strlen_($string);
		$return = '';
		for($x=0;$x<$length;$x++) {
			if($x == 0 || $x == ($length-1)) {
				$return .= $string[$x];
	...
	static function str_slug($title, $separator = '-', $language = 'en')
    {
	...
	static function gen_date_content($date,$lang) { 
		if($date) {
			$ex = explode(' ',$date);
			$set = $ex[0];
			$now = date('Y-m-d');
			$range = (strtotime($now) - strtotime($set))/  ( 60 * 60 * 24 );
			if($lang == 'en') {
				if($range < 2) {
					$return = 'TODAY';
				}else if($range < 3) {
					...
					$return = floor($range/7).' WEEKS AGO';
				}else if($range > 31) {
					$return = floor($range/31).' MONTHS AGO';
				}else if($range > 365) {
					$return = floor($range/365).' YEARS AGO';
				}
			}else if($lang == 'th') {
				if($range < 2) {
					$return = 'วันนี้';
					...
					$return = $range.' วันที่แล้ว';
				}else if($range > 6) {
					$return = floor($range/7).' สัปดาห์ที่แล้ว';
				}else if($range > 31) {
					$return = floor($range/31).' เดือนที่แล้ว';
				}else if($range > 365) {
					$return = floor($range/365).' ปีที่แล้ว';
				}
			}
		} else {
			$return = $date;
	...
	static function gen_date_news($date, $year = '0', $m_lange = 's') { 
		if($date) {
			$ex     = explode('-', $date);
			$month  = str_pad($ex[1],2,'0',STR_PAD_LEFT);
			$day 	= str_pad($ex[2],2,'0',STR_PAD_LEFT);
			if($m_lange == 's') {
				$return = $day.' '.General::gen_short_month($month,'en').', '.($ex[0]+$year);
			} else {
				$return = $day.' '.General::gen_full_month($month,'en').', '.($ex[0]+$year);
			}
		} else {
			$return = $date;
		}
	...
	static function gen_date_news_br($date, $year = '0', $m_lange = 's') { 
		if($date) {
			$ex     = explode('-', $date);
			$month  = str_pad($ex[1],2,'0',STR_PAD_LEFT);
			$day 	= str_pad($ex[2],2,'0',STR_PAD_LEFT);
			if($m_lange == 's') {
				$return = $day.'<br/><span>'.General::gen_short_month($month,'en').', '.($ex[0]+$year).'</span>';
			} else {
				$return = $day.'<br/><span>'.General::gen_full_month($month,'en').', '.($ex[0]+$year).'</span>';
			}
		} else {
			$return = $date;
		}
	...
	static function gen_thai_date($date, $year = '0', $m_lange = 'f') { //month = (f = full month, s = short month) 
		if($date) {
			$ex     = explode('-', $date);
			$month  = str_pad($ex[1],2,'0',STR_PAD_LEFT);
			$day 	= str_pad($ex[2],2,'0',STR_PAD_LEFT);
			if($m_lange == 's') {
				$return = $day.' '.General::gen_short_month($month,'th').' '.($ex[0]+$year);
			} else {
	...
	static function send_sms_now($tel,$message) {
		if($tel) {
			return General::send_sms(env('SMS_THAIBULK_USERNAME'),\Config::get('sms.password'),$tel,$message,'Kaikongwai');
		} 
	}

	static function send_sms($username, $password, $msisdn, $message, $sender = "Kaikongwai", $ScheduledDelivery = "", $force = "premium") {
    ...
    static function generateRandomStringOnly($length = 4) {
	...
	static function generateRandomNum($length = 6) {
	...
	static function gen_cut_date($date, $mode = "his") {
		if($date) {
			$ex     = explode(' ', $date);
			if($mode == 'his') {
				$return = $ex[1];
	...
	static function num2thai($number){
	...
	if(!empty($number[1])){
		if(strlen($number[1]) == 1){$number[1] .= "0";}
		elseif(strlen($number[1]) > 2){
			if($number[1]{2} < 5){$number[1] = substr($number[1], 0, 2);}
			else{$number[1] = $number[1]{0}.($number[1]{1}+1);}
		}
	}
	
	for($i=0; $i<count($number); $i++){$countnum[$i] = strlen($number[$i]);
	if($countnum[$i] <= 7){$var[$i][] = $number[$i];}
	else{$loopround = ceil($countnum[$i]/6);
	
	for($j=1; $j<=$loopround; $j++){
		if($j == 1){$slen = 0;$elen = $countnum[$i]-(($loopround-1)*6);}
		else{$slen = $countnum[$i]-((($loopround+1)-$j)*6);$elen = 6;}$var[$i][] = substr($number[$i], $slen, $elen);}}$nstring[$i] = "";
		
		for($k=0; $k<count($var[$i]); $k++){
			if($k > 0) $nstring[$i] .= $t2[7];$val = $var[$i][$k];$tnstring = "";$countval = strlen($val);
			for($l=7; $l>=2; $l--){
				if($countval >= $l){$v = substr($val, -$l, 1);
					if($v > 0){if($l == 2 && $v == 1){$tnstring .= $t2[($l)];}
					elseif($l == 2 && $v == 2){$tnstring .= $t2[1].$t2[($l)];}
					else{$tnstring .= $t1[$v].$t2[($l)];}
				}
			}
		}
		
		if($countval >= 1){$v = substr($val, -1, 1);
			if($v > 0){
				if($v == 1 && $countval > 1 && substr($val, -2, 1) > 0){$tnstring .= $t2[0];}
				else{$tnstring .= $t1[$v];}}}$nstring[$i] .= $tnstring;}}$rstring = "";
					if(!empty($nstring[0]) || $zerobahtshow == 1 || empty($nstring[1])){
						if($nstring[0] == "") $nstring[0] = $t1[0];$rstring .= $nstring[0]."บาท";}
						if(count($number) == 1 || empty($nstring[1])){$rstring .= "ถ้วน";}
						else{$rstring .= $nstring[1]."สตางค์";}
			return $rstring;
	}

	static function num2eng($num = false)
	{
	    ...
	    return trim(strtoupper(implode(' ', $words)));
	}
